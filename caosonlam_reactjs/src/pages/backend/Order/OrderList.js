...
const OrderList = () => {
    ...
    const handleDestroy = async (id) => {
        if (window.confirm("Bạn có chắc chắn muốn xóa banner này không?")) {
            try {
                const response = await OrderService.destroy(id); // Gọi phương thức delete từ BannerService
                // Kiểm tra xem phản hồi có chứa thuộc tính 'status' không
                if (response && response.data && response.data.status) {
                    setOrders(orders.filter(order => order.id !== id)); // Cập nhật danh sách banner
        ...
        <div>
            <div className='flex flex-row justify-center items-center py-4 border rounded-lg mb-4 px-4 bg-white'>
                <div className='basis-1/2'>
                    <h1 className='text-3xl uppercase text-green-800'>QUẢN LÝ ORDER</h1>
                </div>
                                ...
                                <tr key={index} className='border-b'>
                                    ...
                                    <td className='border px-4 py-2'>
                                        <div className='flex justify-center items-center'>
                                            <img src={`http://localhost:8000/images/orders/${order.image}`} alt={order.name} className='w-24 h-24 object-cover' />
                                        </div>
                                    </td>
                                    <td className='border px-4 py-2'>{order.name}</td>
                                    <td className='border px-4 py-2'>{order.position}</td>
                                    <td className='border px-4 py-2 text-center'>
                                        {jsxStatus}
